...
struct TemplateTests {
                    ...
                    .for(
                        ...
                        [
                            ...
                            .expression(
                                .binary(
                                    .add,
                                    .binary(.add, .identifier("x"), .string(" ")),
                                    .identifier("y")
                                )
                            ), .text("|"),
                        ],
    ...
    @Test("Object subscripts nested")
    func objectSubscriptsNested() throws {
        let string = #"{{ obj['a']['b']['c'] }}"#
        let context: Context = [
            "obj": [
                "a": [
                    "b": [
    ...
    func stringStartswith() throws {
        let string = #"{{ "hello world".startswith("hello") }}"#
        let context: Context = [:]
    ...
    func stringStartswithFalse() throws {
        let string = #"{{ "hello world".startswith("world") }}"#
        let context: Context = [:]
    ...
    func stringEndswith() throws {
        let string = #"{{ "hello world".endswith("world") }}"#
        let context: Context = [:]
    ...
    func stringEndswithFalse() throws {
        let string = #"{{ "hello world".endswith("hello") }}"#
        let context: Context = [:]
    ...
    @Test("int filter with default parameter")
    func intFilterWithDefault() throws {
        let template = try Template("{{ 'invalid'|int(default=99) }}")
        let result = try template.render([:])
        #expect(result == "99", "Should return default value when conversion fails")
    }

    @Test("float filter with default parameter")
    func floatFilterWithDefault() throws {
        let template = try Template("{{ 'invalid'|float(default=1.5) }}")
        let result = try template.render([:])
        #expect(result == "1.5", "Should return default value when conversion fails")
    }

    @Test("max filter with attribute parameter")
    ...
    @Test("min filter with attribute parameter")
    func minFilterWithAttribute() throws {
        ...
        let template = try Template("{{ (users|min(attribute='age')).name }}")
        let result = try template.render(context)
        #expect(result == "Jane", "Should return user with lowest age")
    }

    @Test("select filter without testname parameter")
    ...
    @Test("reject filter without testname parameter")
    func rejectFilterWithoutTestName() throws {
        ...
        let template = try Template("{{ items|reject|join(',') }}")
        let result = try template.render(context)
        #expect(result == ",", "Should reject truthy values, leaving empty string and null")
    }

    @Test("escape filter HTML entity encoding")
    func escapeFilterHTMLEntityEncoding() throws {
    ...
    @Test("Namespace attribute mutations should persist after loops")
    func namespaceAttributeMutationPersistence() throws {
        let template = """
            {% set ns = namespace(is_last_user=false) %}
        ...
        #expect(rendered.trimmingCharacters(in: .whitespacesAndNewlines) == "true")
    }

    @Test("Simple namespace test")
    func simpleNamespaceTest() throws {
        let template = """
        ...
        let rendered = try Template(template).render([:])
        #expect(rendered.trimmingCharacters(in: .whitespacesAndNewlines) == "true")
    }

    @Test("Namespace vs regular variable scoping difference")
    func namespaceVsRegularVariableScoping() throws {
        // Regular variable should NOT persist outside loop
        let regularVarTemplate = """
            {% set found = false %}
            {% for num in [1] %}
            {% set found = true %}
            {% endfor %}
        ...
        let regularResult = try Template(regularVarTemplate).render([:])
        #expect(regularResult.trimmingCharacters(in: .whitespacesAndNewlines) == "false")
